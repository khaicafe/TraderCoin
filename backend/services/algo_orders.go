...
func (ts *TradingService) PlaceAlgoTakeProfit(
	...
	// Determine opposite side for closePosition order
	...
	params.Set("timeInForce", "GTE_GTC")
	params.Set("stopPrice", fmt.Sprintf("%.2f", takeProfitPrice))
	...
	if resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusCreated {
		...
		return OrderResult{Success: false, Error: errorMsg, ErrorDetails: errorResp}
	}
	...
	fmt.Printf("   Symbol: %s\n", orderResp.Symbol)
	fmt.Printf("   Type: %s\n", orderResp.Type)
	fmt.Printf("   StopPrice: %s\n", orderResp.StopPrice)
	fmt.Printf("   ClosePosition: %v\n", orderResp.ClosePosition)
	fmt.Printf("   Status: %s\n\n", orderResp.Status)

	return OrderResult{Success: true, OrderID: strconv.FormatInt(orderResp.OrderID, 10), Symbol: orderResp.Symbol}
}
